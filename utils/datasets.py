...
def qm9_edges(g, e_representation="raw_distance"):
    ...
    #Independent Study Contribution
    for i in range(g.number_of_nodes(), 29):
        g.add_node(i)
...
def get_graph_stats(graph_obj_handle, prop="degrees"):
    ...
    inputs = [int(i * len(graph_obj_handle) / num_cores) for i in range(num_cores)] + [
        len(graph_obj_handle)
    ]
    res = Parallel(n_jobs=num_cores)(
        delayed(get_values)(graph_obj_handle, inputs[i], inputs[i + 1], prop)
    ...
    if "degrees" in prop:
        stat_dict["degrees"] = list(
            set(
                [
                    d
                    for core_res in res
                    for file_res in core_res
                    for d in file_res["degrees"]
                ]
            )
        )
    if "edge_labels" in prop:
        stat_dict["edge_labels"] = list(
            set(
                [
                    ...
                    for core_res in res
                    for file_res in core_res
                    for d in file_res["edge_labels"]
                ]
            )
        )
    if "target_mean" in prop or "target_std" in prop:
        param = np.array(
            [file_res["params"] for core_res in res for file_res in core_res]
        )
    ...
    batch_sizes = np.max(
        np.array(
            [
                ...
                if input_b[2]
                else [len(input_b[1]), len(input_b[1][0]), 0, 0]
                for (input_b, target_b) in batch
            ]
        ),
        axis=0,
    )
