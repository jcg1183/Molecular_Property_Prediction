...
def load_dataset(directory, dataset, subdir="01_Keypoint"):
    ...
    if dataset == "enzymes":
        ...
        (
            ...
            test_graphs,
            test_classes,
        ) = divide_datasets(graphs, classes)
        ...
        train_classes, train_files = read_2cols_set_files(
            join(directory, "Set/Train.txt")
        )
        test_classes, test_files = read_2cols_set_files(join(directory, "Set/Test.txt"))
        valid_classes, valid_files = read_2cols_set_files(
            join(directory, "Set/Valid.txt")
        )

        train_classes, valid_classes, test_classes = create_numeric_classes(
...
def divide_datasets(graphs, classes):
    ...
    for c in uc:
        ...
        va_idx += sorted(
            np.random.choice(
                [x for x in idx if x not in tr_idx], int(0.1 * len(idx)), replace=False
            )
        )
        te_idx += sorted(
            np.random.choice(
                [x for x in idx if x not in tr_idx and x not in va_idx],
                int(0.1 * len(idx)),
                replace=False,
            )
        )
...
def create_graph_mutag(file):
    ...
    g = nx.parse_edgelist(
        edge_list, nodetype=int, data=(("weight", float),), delimiter=","
    )
