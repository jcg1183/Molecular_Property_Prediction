...
parser.add_argument("--fold", default="0", help="current cross fold")

parser.add_argument("--dataset", default="qm9", help="QM9")
parser.add_argument(
    "--datasetPath", default="./data/qm9/dsgdb9nsd/", help="dataset path"
)
...
parser.add_argument(
    "--resume", default="./checkpoint/qm9/mpnn/", help="path to latest checkpoint"
)
...
parser.add_argument(
    "--model",
    type=str,
    help="MPNN model name [MPNN, MPNNv2, MPNNv3, LSTM, MPNNvG]",
    default="MPNN",
)
...
def main(args):
    ...
    print("\tStatistics")
    stat_dict = datasets.get_graph_stats(all_data, ["target_mean", "target_std"])

    # Identify atoms in all files

    data_train.set_target_transform(
        lambda x: datasets.normalize_data(
            x, stat_dict["target_mean"], stat_dict["target_std"]
        )
    )
    
    data_test.set_target_transform(
        lambda x: datasets.normalize_data(
            x, stat_dict["target_mean"], stat_dict["target_std"]
        )
    )
    ...
    if args.model == "MPNNv2":
        ...
        model = LSTM(
            in_n, hidden_state_size, message_size, n_layers, l_target, type=type
        )
    else:
        model = MPNN(
            in_n, hidden_state_size, message_size, n_layers, l_target, type=type
    ...
    for epoch in range(0, args.epochs):
        ...
        if (
            epoch > args.epochs * args.schedule[0]
...
def train(train_loader, model, criterion, optimizer, epoch, evaluation, logger):
    ...
    print(
        "Epoch: [{0}] Avg Error Ratio {err.avg:.3f}; Average Loss {loss.avg:.3f}; Avg Time x Batch {b_time.avg:.3f}".format(
            epoch, err=error_ratio, loss=losses, b_time=batch_time
        )
    )
...
def validate(val_loader, model, criterion, evaluation, logger=None):
    ...
    print(
        " * Average Error Ratio {err.avg:.3f}; Average Loss {loss.avg:.3f}".format(
            err=error_ratio, loss=losses
        )
    )
