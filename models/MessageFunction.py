...
class MessageFunction(nn.Module):
    ...
    def init_mpnn(self, params):
        ...
        learn_modules.append(
            NNet(n_in=params["edge_feat"], n_out=(params["in"] * params["out"]))
        )

        return nn.ParameterList(learn_args), nn.ModuleList(learn_modules), args

    # Message passing algorithm for lstm version
    def m_lstm(self, h_v, h_w, e_vw, opt={}):
        # Matrices for each edge
        edge_output = self.learn_modules[0](e_vw)
        edge_output = edge_output.view(-1, self.args["out"], self.args["in"])
    ...
    def out_lstm(self, size_h, size_e, args):
        return self.args["out"]
